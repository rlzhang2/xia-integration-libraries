...
int main()
{
	...
	chunk_recv_dir = CHUNKS_RECV_DIR;
	...
	while(picoquic_get_cnx_state(connection) != picoquic_state_disconnected) {
		...
		if(bytes_recv > 0) {
			...
			if(picoquic_incoming_packet(client, buffer,
				...
				store_chunk(connection, &callback_context, buffer, bytes_recv, test_cid.c_str(), chunk_recv_dir, procType);
			}
			delta_t = 0;
		}
