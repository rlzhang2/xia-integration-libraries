...
cid_list_t chunk_file(std::string filename, uint32_t chunk_size, std::string cidtype, std::string chunk_loc, uint32_t ttl);
...
std::vector<uint8_t> get_chunkdata(std::string cid, std::string chunk_loc, std::string processType,  size_t cSize);
...
//add functions for application layer api
vector <string> chunk_file_hash(std::string file_name, uint32_t chunk_size, std::string cidtype, std::string chunk_loc);
#endif // _chunkapi_h
