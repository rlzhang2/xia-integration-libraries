...
void put_chunk(picoquic_cnx_t* connection,
                struct callback_context_t* context,std::string chunk_loc, std::string process_type, vector <string> xid_lst)
{
	...
	if(process_type.compare("GET")==0){
	 for (int i=0; i<xid_lst.size(); i++){
			...
			context->loc =chunk_loc; //location that client is to store the receiving chunk
		...
		for (int i=0; i<xid_lst.size(); i++){
			...
			std::vector<uint8_t> tmpChunkData;
                	tmpChunkData = get_chunkdata(xid_lst[i], chunk_loc, process_type, TEST_CHUNK_SIZE);
...
int store_chunk(picoquic_cnx_t* cnx, struct callback_context_t* context,
        uint8_t* bytes, size_t length, string xid_requested, string recv_dir, string process_type) {
        std::cout<<"--------function: "<<__FUNCTION__<<"------"<<endl;
                std::string path, tmp_fs;
                std::string homepath = getenv("HOME");
		auto conf = LocalConfig(CONFFILE);
                #ifdef WORKDIR
                        homepath.assign(conf.get(WORKDIR));
                #endif
                tmp_fs = recv_dir.empty() ? homepath + conf.get(CHUNKS_RECV_DIR) : recv_dir;
                size_t found;
                int type_offset=0;
		...
		std::pair<string, vector<uint8_t> > xid_tmp;
			...
			if (length > type_offset +1 ) {//since both CID and NCID are defined as a fixed length
				...
				printf("Invalid data format on put operation!!"); //incorrect ncid format to put
                        	return -1;
			}
                } else {//GET
                ...
                picoquic_call_back_event_t event, void *callback_context)
{
	...
	if (!context->xid.empty()) { //not the first initial stream
		...
		std::cout<<"Check the context location: "<<context->loc.c_str()<<endl;
	...
	switch(event) {
			...
			printf("Callback: StreamData\n");
				...
				printf("Check store loc %s \n", context->loc.c_str());
                                if ((stream_id !=0) &&
					store_chunk(cnx, context,bytes, length, context->xid[(stream_id-1)/2], context->loc, process_type) ==0){
                                        b_validate = true;
                                }
...
void get_chunk_data (struct addr_info_t &test_from_addr, struct addr_info_t &test_to_addr, vector <string> xid_lst,
                        ...
                        uint64_t current_time, std::string chunk_loc, std::string process_type, LocalConfig &conf){
	...
	if(process_type.compare("PUT")==0) {
	 ...
	 //If 0RTT is available, start a stream
		...
		put_chunk(quic_conn, &callback_context,chunk_loc, process_type,xid_lst);
