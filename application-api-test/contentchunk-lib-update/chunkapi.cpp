...
std::vector<uint8_t> get_chunkdata(std::string cid,string chunk_loc, string processType, size_t cSize)
{
    ...
    //Check to see if CID content is available on xcache local:
     ...
     //Use chunk_loc client passed in 
     std::string CID_path = chunk_loc + cid;
     std::cout<<"CID path "<<CID_path.c_str()<<std::endl;
 ...
 * */
std::string write_chunk(const unsigned char *buf, uint32_t byte_count, string chunk_loc)
{
	unsigned char digest[SHA_DIGEST_LENGTH];
	char digest_string[SHA_DIGEST_LENGTH*2+1];
	std::string homepath = getenv("HOME");
		...
		auto confile = LocalConfig(CONFFILE);
		 homepath.assign(confile.get(WORKDIR));
     	#endif
	std::string content_dir = chunk_loc.empty() ? homepath + CHUNKS_DIR : chunk_loc;
 ...
 **/
std::string write_chunk(const unsigned char *buf, uint32_t byte_count, std::string publisher_name, std::string content_name, string chunk_loc) 
{
	std::string homepath = getenv("HOME");
     	#ifdef WORKDIR
		 auto confile = LocalConfig(CONFFILE);
...
**/
cid_list_t make_chunks(std::string filename, uint32_t chunk_size, string cidtype, string chunk_loc)
{
	struct stat fs;
	cid_list_t cids;
...
if (stat(filename.c_str(), &fs) == 0) { //if file is existing
	...
	// Nov29. Application add cidtype to seperate NCID, or CID
  	...
  	if (found_p!=std::string::npos) {
		...
		while (!feof(f)) {
			if ((byte_count = fread(buf, sizeof(unsigned char), chunk_size-128, f)) > 0) {
				if ((cid = write_chunk(buf, byte_count, publisher_name, content_name, chunk_loc)) == "") {
					fclose(f);
					cids.clear();
					return cids;
					}
		...
		while (!feof(f)) {
                        if ((byte_count = fread(buf, sizeof(unsigned char), chunk_size, f)) > 0) {
                                if ((cid = write_chunk(buf, byte_count, chunk_loc)) == "") {
                                        fclose(f);
                                        cids.clear();
                                        return cids;
                                	}
...
cid_list_t put(const char *buffer, uint32_t size, uint32_t chunk_size, uint32_t ttl)
{
	...
	for (uint32_t i = 0; i < num_chunks; ++i) {
		...
		if ((cid = write_chunk(p, count, content_dir)) == "") {
			cids.clear();
			return cids;
		}
		cids.emplace_back(cid);
...
//add application param
cid_list_t chunk_file(std::string filename, uint32_t chunk_size, string cidtype, string chunk_loc, uint32_t ttl)
{
        cid_list_t cids;
        std::cout <<"Call from chunkapi to make chunks"<<endl;
        cids = make_chunks(filename, chunk_size, cidtype, chunk_loc);
...
//application layer 
/* This function used to chunk a file with its location, chunksize and the location to store the chhunks
 *  from application config, chunksize will default to 300 if not defined in configuration file
 * param file name
 * return a list of chunkcids, 
 */
vector <string> chunk_file_hash(string file_name, uint32_t chunk_size, string cidtype, string chunk_loc){
        std::cout<<"Build list of  chunkXIDs based on application config ----"<<std::endl;
        vector <string> xidLst;

	//didn't get chunk size from application
	uint32_t c_size = chunk_size==0 ? TEST_CHUNK_SIZE : chunk_size;
	if (file_name.empty()) {
		cout << "ERROR: content file name missing"  << endl;
		return {};
	}
	/*application uses the abs path of the file
	std::string homepath = getenv("HOME");
        #ifdef WORKDIR
                 homepath.assign(conf.get(WORKDIR));
        #endif
        std::string file = homepath + f_path +file_name;
	*/
	cout<<"file name :"<<file_name.c_str() <<" size :"<<c_size<<endl;

        cid_list_t chunkIDs = chunk_file(file_name, c_size, cidtype, chunk_loc,  default_ttl);
        for (int i=0; i<chunkIDs.size(); i++) {
                std::string tmp_xid = chunkIDs[i]; //Do I need the first part of string
                xidLst.emplace_back(tmp_xid);
        }
	printf("Completed creating chunks \n");
	for (int i=0; i<xidLst.size(); i++){
               std::cout<<"XID: "<< xidLst[i].c_str()<<endl;
                }
