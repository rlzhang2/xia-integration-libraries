...
void put_chunk(picoquic_cnx_t* connection,
                struct callback_context_t* context, int numOfxids, std::string chunk_loc, std::string process_type, vector <string> xid_lst);
int store_chunk(picoquic_cnx_t* cnx, struct callback_context_t* context,
        uint8_t* bytes, size_t length, std::string xid_requested, std::string recv_loc, std::string process_type);
